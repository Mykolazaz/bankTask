...
create_model_data <- function(data, outcome_var, exclude_vars = NULL) {
  if (!is.null(exclude_vars)) {
    data <- data %>%
      select(-any_of(exclude_vars))
  }
  
  outcome_data <- data %>% select(all_of(outcome_var))
  predictors <- data %>% select(-all_of(outcome_var))

  vars_to_dummy <- predictors %>%
    select(where(~ !is.numeric(.) | n_distinct(.) > 2)) %>%
    colnames()
    
  numeric_data <- predictors %>%
    select(where(is.numeric) & !any_of(vars_to_dummy))

  dummy_data <- predictors %>%
    select(any_of(vars_to_dummy)) %>%
    dummy_cols(remove_selected_columns = TRUE, remove_first_dummy = TRUE)
  
  model_data <- bind_cols(numeric_data, dummy_data, outcome_data)
...
```{r}
dummy_full <- create_model_data(bank_data, outcome_var = "subscribed", exclude_vars = c("day", "month", "age"))
```

And finally, we can run the model with all the variables that are available before contacting the client and output the results.
...
Looking at the model summary, we can say that:
1. Grouping of the age variable turned out to be meaningful;
2. The job type isn't influential in the model and certain professions should be grouped by their complexity;
3. Balance, even when turned into an indicator variable, is not statistically significant;

Now we remove variables that are not statistically meaningful to the model and produce a second much simpler model.

```{r}
dummy_full_2 <- create_model_data(bank_data, outcome_var = "subscribed", exclude_vars = c("day", "month", "age", "balance", "balance_above_250", "poutcome", "previous"))
...
Although, with the statistically insignificant parameters removed, our logistic regression model's AUC is lowered to 0,8908 from 0,9009, the model becomes much simpler which is preferred.
...
The sensitivity (true positive) of the model is quite low. Only 25,3% of clients who would subscribe to a deposit are being recognized as "subscribers".
...
By lowering the threshold down to 0.12, true positives are being recognized with 81,28% accuracy (up from 25,3%) and the specificity is raised to 81,32% (from 75,92%).

# Conclusion

1.  The logistic regression model accuracy score is 0.8128 (with threshold adjusted). True positive rate is 81,28%.
2.  Most important parameters for choosing a potential bank deposit subscriber are contact type and whether the client has borrowed a loan.
